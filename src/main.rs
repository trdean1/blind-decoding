...
fn run_reps() { //{@
    ...
    let dims = vec![(6, 15)];
    */
    let mut results: Vec<((usize, usize), u64, u64, u64, u64, f64)> = dims.iter()
        .map(|&d| (d, 0, 0, 0, 0, 0.0))
        .collect::<Vec<_>>();
    ...
    for _iter in 0 .. reps_per * dims.len() {
        ...
        match single_run(&y) {
            ...
            Ok(ft) => {
                ...
                if equal_atm(&uy, &x) {
                    ...
                    match a.try_inverse() {
                        None => println!("UNEQUAL: Cannot take a^-1"),
                        Some(inv) => println!("UNEQUAL: u = {}a^-1 = {}", 
                                ft.state.u, inv),
                    };
        ...
        res.5 += elapsed.as_secs() as f64 + elapsed.subsec_nanos() as f64 * 1e-9;
    }

    for ref res in results.iter() {
        let mut output = 
            format!("n = {}, k = {:2}: success = {} / {} (pm1 = {}, err = {}), ",
                    (res.0).0, (res.0).1, res.2, res.1, res.3, res.4);
        output += &format!("mean_time_per = {:.5e}", res.5 / res.1 as f64);
        println!("{}", output);
    }
} //@}
//{@
...
fn single_run(y: &na::DMatrix<f64>) -> Result<FlexTab, FlexTabError> { //{@
    ...
    loop {
        ...
        ft = match FlexTab::new(&bfs, &y, ZTHRESH) {
            ...
            Err(e) => match e {
                ...
                FlexTabError::GoodCols => {
                    println!("Insufficient good cols, retrying...");
                    attempts += 1;
                    if attempts > LIMIT {
        ...
        match ft.solve() {
            Ok(_) => break,
            Err(e) => match e {
                FlexTabError::LinIndep => println!("LinIndep"),
                FlexTabError::StateStackExhausted 
                    => println!("State stack exhausted, retrying..."),
                FlexTabError::TooManyHops
                    => println!("Too many hops, retrying..."),
                _ => return Err(e),
            },
        }
    }
    ...
    debug!("FlexTab n = {}: {}, visited = {}", ft.n, 
            if ft.ybad.is_some() { "REDUCED" } else { "FULL" },
            ft.visited.len());
    ...
    match ft.ybad {
        Some(_) => {
            println!("Reduced: now need to solve...");
...
fn rand_pm1_matrix(nrows: usize, ncols: usize) -> na::DMatrix<f64> { //{@
    ...
    na::DMatrix::from_row_slice(nrows, ncols, &data)
} //@}
#[allow(dead_code)]
//{@
...
impl Constraint { //{@
    ...
    fn push_addend(&mut self, uyvar: usize, prodmul: i64, coeff: f64) {
        self.addends.push((uyvar, prodmul, coeff));
    }

    //{@
    /// Check if the constraint is still satisfied.
    //@}
    fn check(&self, prod: &na::DMatrix<f64>, rownum: usize, zthresh: f64) -> bool {
        if self.addends.len() == 0 {
            return true;
        }

        let row = prod.row(rownum);
        let total = self.addends.iter().fold(0.0, |total, &addend| {
            let (uyvar, prodmul, coeff) = addend;
            let contrib = (1.0 + (row[uyvar] * prodmul as f64)) * coeff;
            total + contrib
        });
...
impl FlexTab { //{@
    ...
    fn add_extra_constr(&mut self, csetnum: usize, rownum: usize, //{@
        ...
        let base = (2 * self.n * self.n) + (csetnum * 2 * self.k);
        ...
        for &v in slackvars {
            let coeff = row[v];
            let prodmul = if v & 0x1 == 1 { 1 } else { -1 };
            let uyvar = (v - base) >> 1;
            con.push_addend(uyvar, prodmul, coeff);
        }
    ...
    fn hop(&mut self) -> Result<(), FlexTabError> { //{@
        ...
        loop {
            ...
            if !self.eval_vertex() {
                ...
                println!("End obj = {:.5}", self.state.obj);
                break;
            }
    ...
    fn eval_vertex(&self) -> bool { //{@
        ...
        for (row, ref cset) in self.extra_constr.iter().enumerate() {
            for (cnum, ref c) in cset.iter().enumerate() {
                ...
                // in the row.
                if !c.check(&self.state.uy, row, self.zthresh) {
                    println!("check fail: row = {}, cnum = {}", row, cnum);
                    return false;
                }
            }
        }
    ...
    fn mark_visited(&mut self) { //{@
        ...
        self.set_flip_grad();
        self.set_uybad();
        // Vertex needs to have been set in flip.
        ...
        for i in 0 .. self.n {
            for j in 0 .. self.n {
                ...
                for u_j in 0 .. self.n {
                    ...
                    // Calc effect of movinng this slackvar on the U-value, then
