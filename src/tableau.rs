...
use super::sparsemtx::{SparseTMatrix};
...
impl fmt::Display for FlexTab { //{@
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for i in 0 .. self.state.rows.nrows() {
            let mut line = vec![];
            for j in 0 .. lastcol {
                let v = self.state.rows[(i, j)];
                if v.abs() > self.zthresh {
                    line.push(format!("{:5.2}x_{}", v, j));
                }
            }
            let line = line.join(" + ");
            s += &format!("{} == {:5.2}\n", line, self.state.rows[(i, lastcol)]);
        }
        // XXX: end new code
...
impl FlexTab { //{@
    ...
    fn to_simplex_form(&mut self) -> Result<(), FlexTabError> { //{@
        ...
        for groupnum in 0 .. self.n {
            ...
            //    .collect::<HashSet<_>>();
            let mut avail_basic = (0 .. self.n).collect::<HashSet<_>>();
            ...
            for j in 0 .. self.k {
                ...
                for &useable in avail_basic.iter() {
                    let row_val = self.state.rows.index_block( groupnum, 2*j, 2*useable);
                    
                    if row_val.abs() > self.zthresh {
                        uvar = Some(useable);
                        break;
                    }
                }
    ...
    fn set_basic(&mut self, groupnum: usize, j: usize, pivot_row: usize) //{@
            -> Result<(), FlexTabError> {
        let row_shift = 2*self.k*groupnum;
        let col_shift = 2*self.n*groupnum;
        ...
        //let divisor = self.state.rows[(pivot_row, tgtvar)];
        ...
        for i in baserow .. limrow {
            ...
            //let mult = -1.0 * self.state.rows[(i, tgtvar)];
                ...
                if uvars.len() > 0 {
                    ...
                    match self.urows[uvar] {
                        ...
                        None => {
                            ...
                            self.urows[uvar] = 
                                if self.state.rows[(row, 2 * uvar)] > 0.0 {
    ...
    fn categorize_row(&self, rownum: usize) -> (Vec<usize>, Vec<usize>) { //{@
        ...
        for j in low_u .. high_u {
            if self.state.rows[(rownum, j)].abs() > self.zthresh {
                uvars.push(j);
            }
        }
    ...
    */
    ...
    fn flip(&mut self, v: usize) { //{@
        ...
        for i in base .. top {
            ...
            let old_zero_coeff = self.state.rows[(maniprow, old_zero)];

            // Execute pivot.
            //let mut row = self.state.rows.row_mut(maniprow);
            let rhs = self.state.rows.ncols() - 1; //let rhs = row.len() - 1;
            self.state.rows[(maniprow, rhs)] -= old_zero_coeff * 2.0;
            self.state.rows[(maniprow, old_zero)] = 0.0;
            self.state.rows[(maniprow, new_zero)] = -old_zero_coeff;

            // Set the relevant value of x.
            if self.state.rows[(maniprow, rhs)] >= 0.0 {
                self.state.x[xplus] = self.state.rows[(maniprow, rhs)];
                self.state.x[xminus] = 0.0;
            } else {
                self.state.x[xplus] = 0.0;
                self.state.x[xminus] = -self.state.rows[(maniprow, rhs)];
            }
        }
    } //@}
        ...
        for i in 0 .. self.n {
            ...
            for j in 0 .. self.n {
                ...
                for u_j in 0 .. self.n {
                    ...
                    //let sign = self.state.rows[(urow, 2 * u)];
                    //println!("n = {}, row = {}, col = {}", i, urow - prod_ni, 2 * u_j);
                    //println!("i = {}, j = {}, u_j = {}, u = {}, urow = {}", i, j, u_j, u, urow);
                    //println!("\trow = {}, col = {}\n", urow - 2 * i * self.k, 2 * u_j);
                    let sign = self.state.rows.index_block(i, urow - 2 * i * self.k, 2 * u_j);

                    // Get coefficient of the slackvar that will go 0 -> 2.
                    //let coeff = self.state.rows[(urow, self.state.vmap[v])];
                    let coeff = self.state.rows.index_sparse(i, urow - 2 * i * self.k,
                            self.state.vmap[v] - 2 * self.n * self.n - 2 * i * self.k);
